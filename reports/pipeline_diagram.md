...
fixed-size windows. Embeddings are currently generated with the Bangla-finetuned
`shihab17/bangla-sentence-transformer`; the previous MiniLM baseline is kept commented in
`configs/config.yaml` for quick rollback. See `configs/design_choices.md` for the full per-stage justification.
